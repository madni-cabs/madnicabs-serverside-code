...
rentRouter.post("/add", async (req, res) => {
    const { pickupLocation, dropLocation, distance, rent } = req.body;
    const id = generateId();
  
    const query = `
      INSERT INTO renttable (id, pickupLocation, dropLocation, distance, rent)
      VALUES (@id, @pickupLocation, @dropLocation, @distance, @rent)
    `;
    ...
    try {
      ...
      res.status(201).json({ id, pickupLocation, dropLocation, distance, rent });
      console.log(res);
    } catch (err) {
...
rentRouter.put("/update/:id", async (req, res) => {
  ...
  const query = `
    UPDATE renttable
    SET pickupLocation='${pickupLocation}', dropLocation='${dropLocation}', distance='${distance}', rent='${rent}'
    WHERE id='${id}'
  `;
  ...
  try {
    const request = new sql.Request();
    await request.query(query);
    res.status(200).json({ id, pickupLocation, dropLocation, distance, fare });
  } catch (err) {
...
rentRouter.get("/locations/:pickupLocation", async (req, res) => {
  ...
  const query = `
      SELECT DISTINCT dropLocation FROM renttable WHERE pickupLocation=@pickupLocation
  `;
...
rentRouter.get("/rent-distance", async (req, res) => {
  ...
  const query = `
      SELECT rent, distance FROM renttable 
  ...
  try {
      ...
      console.error('Error retrieving rent and distance:', err);
      res.status(500).json({ error: "Error retrieving rent and distance" });
  }
});
